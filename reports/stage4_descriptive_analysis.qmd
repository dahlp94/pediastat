...
Stage 5 locked the inferential models, coding, missing-data strategy, and
diagnostics before any Cox fit. See `docs/inferential_model_specification.md`.
...
# 12. Subsequent Inferential Plan

Stage 5 froze the inferential plan before regression. The locked specification is `docs/inferential_model_specification.md`. In brief:

- Primary clinical Cox model: age5 + sex + log2(WBC) + risk group
- Secondary molecular/cytogenetic Cox model without risk group
- Multiple imputation is primary; complete case is sensitivity
- No Cox model, hazard ratio, or predictor-specific survival analysis is contained in this Stage 4 report
